...
int USB3WriteControl(int sid, uint8_t bRequest, uint16_t wValue, uint16_t wIndex, unsigned char *data, uint16_t wLength)
{
  ...
  libusb_device_handle *devh = nkusb_get_device_handle(CALDAQ_VENDOR_ID, CALDAQ_PRODUCT_ID, sid);
  if (!devh) {
...
int CALDAQopen(int sid)
{
  ...
  fprintf(stdout, "Info: open_device: opening device Vendor ID = 0x%X, Product ID = 0x%X, Serial ID = %u\n",
                   CALDAQ_VENDOR_ID, CALDAQ_PRODUCT_ID, sid);
  ...
  while ((dev = devs[i++])) {
    ...
    if (desc.idVendor == CALDAQ_VENDOR_ID && desc.idProduct == CALDAQ_PRODUCT_ID)  {
  ...
  // claim interface
  handle_interface_id(&ldev_open, CALDAQ_VENDOR_ID, CALDAQ_PRODUCT_ID, sid, 0, kInterfaceClaim);

  if (!nopen_devices)
    return -1;

  devh = nkusb_get_device_handle(CALDAQ_VENDOR_ID, CALDAQ_PRODUCT_ID, sid);
  if (!devh) {
...
// close CALDAQ
void CALDAQclose(int sid)
{
  handle_interface_id(&ldev_open, CALDAQ_VENDOR_ID, CALDAQ_PRODUCT_ID, sid, 0, kInterfaceRelease);
  remove_device_id(&ldev_open, CALDAQ_VENDOR_ID, CALDAQ_PRODUCT_ID, sid);
}
...
unsigned long CALDAQread_RUN(int sid)
{
