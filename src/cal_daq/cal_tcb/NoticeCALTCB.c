...
unsigned long CALTCBread_DRAMTEST(int sid, unsigned long mid);
...
int CALTCBread_DRAM_ALIGN(int sid, unsigned long mid);
void CALTCBreset_REF_CLK(int sid, unsigned long mid);
void CALTCBwrite_DRAMDLY(int sid, unsigned long mid, int data);
void CALTCBwrite_DRAM_BITSLIP(int sid, unsigned long mid);
...
{
  TCBWrite(sid, 0, 0x00, 0x08);
}

// start DRS
void CALTCBstart_DRS(int sid)
{
  TCBWrite(sid, 0, 0x00, 0x10);
}

// stop DRS
void CALTCBstop_DRS(int sid)
{
  TCBWrite(sid, 0, 0x00, 0x20);
}

// read DAQ status
unsigned long CALTCBread_RUN(int sid)
{
  return TCBReadReg(sid, 0, 0x00);
}

// read DAQ link status
void CALTCBread_LINK(int sid, unsigned long *data)
{
  data[0] = TCBReadReg(sid, 0, 0x01);
  data[1] = TCBReadReg(sid, 0, 0x02);
}
...
void CALTCBread_MID(int sid, unsigned long *data)
{
  ...
  for (i = 0; i < 40; i ++) 
    data[i] = TCBReadReg(sid, 0, 0x03 + i);
}
...
// read high voltage
float CALTCBread_HV(int sid, unsigned long mid, unsigned long ch, int univ)
{
  unsigned long data;
  float value;
  unsigned long addr = 0x02;
...
unsigned long CALTCBread_DRAMTEST(int sid, unsigned long mid)
{
  return TCBReadReg(sid, mid, 0x07);
}
...
void CALTCBwrite_TRIGGER_LATENCY(int sid, unsigned long mid, unsigned long data)
{
  TCBWrite(sid, mid, 0x0A, data);
}

// read trigger latency
unsigned long CALTCBread_TRIGGER_LATENCY(int sid, unsigned long mid)
{
  return TCBReadReg(sid, mid, 0x0A);
}

// write run number
void CALTCBwrite_RUN_NUMBER(int sid, unsigned long mid, unsigned long data)
{
  TCBWrite(sid, mid, 0x0B, data);
}

// read run number
unsigned long CALTCBread_RUN_NUMBER(int sid, unsigned long mid)
{
...
// read down-sampling
unsigned long CALTCBread_DOWN_SAMPLING(int sid, unsigned long mid)
{
...
void CALTCBwrite_DRAMDLY(int sid, unsigned long mid, int data)
{
  TCBWrite(sid, mid, 0x24, data);
}

// write DRAM input bitslip
void CALTCBwrite_DRAM_BITSLIP(int sid, unsigned long mid)
{
  TCBWrite(sid, mid, 0x25, 0);
}
...
//printf("dly = %d, value = %X\n", dly, value);      
  ...
  // set delay
  CALTCBwrite_DRAMDLY(sid, mid, gdly);
