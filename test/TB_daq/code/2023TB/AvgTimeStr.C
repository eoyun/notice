...
int AvgTimeStr(const int runnum, const int Mid, const int channel, const int channel2)
{
  int ch_to_plot;
  int ch_to_plot2;
  FILE *fp; 
  ...
  TH1F *plot2 = new TH1F("Avg.Time.Str2", "Avg.Time.Str;ADC;ADC", 1024, 0., 1024.);
  ...
  //sprintf(filename,"/Volumes/HDD_16TB_3/Run_%d/Run_%d_Wave/Run_%d_Wave_MID_%d/Run_%d_Wave_MID_%d_FILE_0.dat",runnum,runnum,runnum,Mid,runnum,Mid);
  ...
  for ( evt = 0; evt < nevt; evt++ ) {
    ...
    for ( i = 0; i < 1023; i++ ) {
      ...
      plot2->Fill(i, adc[i * 32 + ch_to_plot2]);
    }
