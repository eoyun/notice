...
int show_fast_sipm_55(const int runnum)
{
	int ch;
	FILE *fp[13];
	int file_size;
	...
	//scanf("%d", &channel);
	//ch_to_plot = ch;
	sprintf(outname,"sipm_%d_5by5_energyslect.root",runnum);
...
//	for (filenum=0;filenum < 200;filenum++){
	for (mid=3;mid<16;mid++){
    	...
    	sprintf(filename,"/Volumes/HDD_16TB_1/HDD_Run_%d_validated/Run_%d_Fast/Run_%d_Fast_MID_%d/Run_%d_Fast_MID_%d_FILE_0.dat",runnum,runnum,runnum,mid,runnum,mid);
	...
	std::map<int, std::vector<std::pair<int, int>>> firingMap;
	...
	for (evt = 0; evt < nevt; evt++) {
		...
		for (mid = 3;mid < 16 ; mid++){
    		...
    		for (ch=0; ch<32; ch++){
				...
				if ((mid==8&&ch==22)||(mid==6&&ch==17)||(mid==6&&ch==18)||(mid==5&&ch==9)||(mid==7&&ch==29)||(mid==7&&ch==30)||(mid==7&&ch==21)||(mid==6&&ch==5)||(mid==6&&ch==6)||(mid==8&&ch==21)||(mid==5&&ch==19)||(mid==5&&ch==10)||(mid==8&&ch==9)){
					...
					if( energy_c>1000 ) { 
						time_flag_C=1;
						if ( firingMap.find(evt) == firingMap.end() ) {
							std::vector<std::pair<int, int>> tmpVec;
							tmpVec.push_back(std::make_pair(mid, ch+1));
							firingMap.insert(std::make_pair(evt, tmpVec));
						} else {
							firingMap.find(evt)->second.push_back(std::make_pair(mid, ch+1));
						}
					}
		...
		if (time_flag_C+time_flag_S==2){
			select_evt+=1;
			// printf("evt number is %d\n",evt);
			// printf("C energy is %d| S energy is %d\n",energy_total_c,energy_total_s);
			// plot_C->Fill(energy_total_c);
	    	// plot_S->Fill(energy_total_s);
		} else {
			firingMap.erase( firingMap.find(evt), firingMap.end()  );
		}
	}
	
	for ( auto firingEvtInfo : firingMap )
		for ( int i = 0; i < firingEvtInfo.second.size(); i++ )
			printf("%d %d %d\n", firingEvtInfo.first, firingEvtInfo.second.at(i).first, firingEvtInfo.second.at(i).second);

    for(mid=3;mid<16;mid++){ 
	    fclose(fp[mid-3]);
	}
//	}
    printf("total selected evt is %d\n",select_evt);
	//c1->cd(1);
	// plot_C->Write();
	//c1->cd(2);
	//plot_S->Write();
