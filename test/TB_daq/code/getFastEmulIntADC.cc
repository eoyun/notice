...
int getFastEmulIntADC(const int runnum, const int Mid, const int channel, const int RE, const int Interval)
{
  ...
  TH1F* intADCdis = new TH1F("ADC", "Int. ADC;Int. ADC;# evts", 1024, -1024, 81920);
  std::vector<float> tmpWave;
  intADCdis->Sumw2();
  intADCdis->SetLineColor(kBlack);
  for (ifile=0;ifile<200;ifile++){  
    sprintf(filename,"/Users/yhep/scratch/YUdaq/SSD/SSD_Run_%d/Run_%d_Wave/Run_%d_Wave_MID_%d/Run_%d_Wave_MID_%d_FILE_%d.dat",runnum,runnum,runnum,Mid,runnum,Mid,ifile);
