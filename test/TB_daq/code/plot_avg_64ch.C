...
int plot_avg_64ch(const int runnum, const int Mid1, const int Mid2, const int min, const int max)
{
  char filename1[100];
  char filename2[100];
...
//    ch_to_plot = channel - 1;
  ...
  for( i = 0 ; i < 64 ; i ++)
  {
     plot[i] = new TH1F(Form("plot%d",i+1), Form("%d", i+1), 1023,0,1023);
     plot[i]->SetStats(0);  
  }
...
//		if(j==21 || j == 22 || j ==23) cout << j << " th channel : " << adc[i * 32 + j]<< endl;
         if (j<32) plot[j]->Fill(i,(double) adc1[i * 32 + j]/nevt);
         else plot[j]->Fill(i,(double) adc2[i * 32 + j-32]/nevt);
      }
    }
