...
int show_wave_remove(const int runnum, const int Mid, const int channel, const int min, const int max)
{
  //int channel;
  int ch_to_plot;
  FILE *fp;
  ...
  // TFile *file = new TFile("wave_peak_range_run_" + (TString)std::to_string(runnum) + ".root", "RECREATE");
...
//  fp = fopen("cal_sipm_pmt_10k.dat", "rb");
  ...
  for (evt = 0; evt < nevt; evt++) {
    ...
    double peak1 = 0;
    double peak2 = 0;
    peak1 = *std::min_element(wave.begin() + 200, wave.begin() + 300);
    peak2 = *std::min_element(wave.begin() + 600, wave.begin() + 1000);
    
    if ( peak1 > 3000.) 
      continue;
      //if ( peak1 < 3300. )
      //continue;
    //if ( peak1 < 3300. && peak2 > 3200. )
      //continue;
  ...
  std::cout << "alive evts : " << alive_evt << std::endl;
