...
int show_wave_200ADC_over(const int runnum, const int Mid, const int ch1, const int ch2, const int min, const int max)
{
  //int channel;
  int ch_to_plot;
  FILE *fp;
...
//  fp = fopen("cal_sipm_pmt_10k.dat", "rb");
  ...
  for (evt = 0; evt < 5000; evt++) {
    ...
    for (i = 0; i < 1023; i++) {
      wave1.push_back(adc[i * 32 + ch1 - 1]);
      wave2.push_back(adc[i * 32 + ch2 - 1]);
    }
    ...
    ped1 = std::accumulate(wave1.begin(), wave1.begin() + 100, 0.) / 100.; 
    // ped2 = std::accumulate(wave2.begin(), wave2.begin() + 100, 0.) / 100.; 
    peak1 = *std::min_element(wave1.begin() + 100, wave1.begin() + 300);
    // peak2 = *std::min_element(wave2.begin() + 100, wave2.begin() + 300);

    if ( (ped1 - peak1) > 200. )
      continue;

    for (i = 0; i < 1023; i++) {
      plot1->Fill(i, adc[i * 32 + ch1 - 1]);
      plot2->Fill(i, adc[i * 32 + ch2 - 1]);
    }
