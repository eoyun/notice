...
int show_wave_specific(const int runnum, const int Mid, const int channel1, const int channel2, const int channel3, const int channel4, const int min, const int max, const int peakThres, const int trigPattern)
{ 
  //int channel;
  int ch_to_plot;
  FILE *fp;
  ...
  TH1F *plot1 = new TH1F("plot1", "Waveform", 1000, 0, 1000); 
  TH1F *plot2 = new TH1F("plot2", "Waveform", 1000, 0, 1000); 
  TH1F *plot3 = new TH1F("plot1", "Waveform", 1000, 0, 1000); 
  TH1F *plot4 = new TH1F("plot2", "Waveform", 1000, 0, 1000);   
  plot1->SetStats(0);
  plot2->SetStats(0);
  plot3->SetStats(0);
  plot4->SetStats(0);
...
//  fp = fopen("cal_sipm_pmt_10k.dat", "rb");
  ...
  for (evt = 0; evt < nevt; evt++) {
    ...
    plot1->Reset();
    plot2->Reset();
    std::vector<short> aWave;
    for (i = 1; i < 1000; i++) {
      plot1->SetBinContent(i, adc[i * 32 + channel1 - 1]);
      plot2->SetBinContent(i, adc[i * 32 + channel2 - 1]);
      plot3->SetBinContent(i, adc[i * 32 + channel3 - 1]);
      plot4->SetBinContent(i, adc[i * 32 + channel4 - 1]);
      // aWave.push_back(adc[i * 32 + ch_to_plot]);
    }
    plot1->GetYaxis()->SetRangeUser(min,max); plot1->SetLineColor(kRed);
    plot2->GetYaxis()->SetRangeUser(min,max); plot2->SetLineColor(kBlue);
    plot3->GetYaxis()->SetRangeUser(min,max); plot3->SetLineColor(kTeal-2);
    plot4->GetYaxis()->SetRangeUser(min,max); plot4->SetLineColor(kSpring+4);
    plot1->Draw("hist");
    plot2->Draw("hist SAME");
    plot3->Draw("hist SAME");
    plot4->Draw("hist SAME");
    
    // float ped = std::accumulate(aWave.begin() + 1, aWave.begin() + 51, 0.) / 50.;
    // if ( ped - *std::min_element(aWave.begin() + 175, aWave.begin() + 275) < peakThres )
    //   continue;
