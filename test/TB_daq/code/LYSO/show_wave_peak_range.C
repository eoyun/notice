...
int show_wave_peak_range(const int RunNum, const int runend, const int Mid, const int ch, const int rangeFirst, const int rangeSecond, const int trigPattern)
{
  int channel;
  int ch_to_plot;
  FILE *fp;
    ...
    for (j=0;j<200;j++){
      ...
      for (evt = 0; evt < nevt; evt++) {
		...
		//if ((ch==12&&ped-peak>1200)) {fprintf(fp_cut,"%d\n",evt);printf("peak value is %d\n",ped-peak);}
      	ped = std::accumulate(aWave.begin(), aWave.begin() + 100, 0.) / 100.;
        peak = *std::min_element(aWave.begin() + rangeFirst, aWave.begin() + rangeSecond);
        hpeak->Fill(ped-peak);
