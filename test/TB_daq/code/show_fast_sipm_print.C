...
int show_fast_sipm_print(const int runnum)
{
	int ch;
	FILE *fp[13];
	int file_size;
	...
	//scanf("%d", &channel);
	//ch_to_plot = ch;
    sprintf(outname,"sipm_%d.root",runnum);
    TFile *fp_root = new TFile(outname,"recreate");  
    TCanvas *c1 = new TCanvas("c1", "CAL DAQ", 800, 800);
    c1->Divide(1, 2);
	TH1F *plot_C = new TH1F("plot_c", "Energy_c", 1000, -200000, 200000); 
	TH1F *plot_S = new TH1F("plot_s", "Energy_s", 1000, -200000, 200000); 
	...
	for (evt = 0; evt < nevt; evt++) {
		...
		for (mid = 3;mid < 16 ; mid++){
    		...
    		for (ch=0; ch<32; ch++){
				...
				//	printf("evt num is %d\n",evt);
         			if (evt==674) printf("mid : %d | ch : %d | energy %d | ceren\n",mid,ch,energy_c);
                ...
                if ((mid>8&&!(mid==11))||(mid==11&&ch<8)){
            		...
            		energy_s = energy_s + (data[mid - 3][ch * 3] & 0xFFFF);
         			if (evt==674) printf("mid : %d | ch : %d | energy %d | scint\n",mid,ch,energy_s);
