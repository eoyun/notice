...
int calADC(const int runnum, const int Mid, const int channel, const int min, const int max, const double time)
{
  int ch_to_plot;
  FILE *fp;
  FILE *fp2;
  ...
  TH1F *intADCHist = new TH1F("calADC", "calibration;MeV;evts", 1000, 0, 2);
  ...
  for ( ifile = 0; ifile < 200; ifile++ ) {  
    ...
    for (evt = 0; evt < nevt; evt++) {
      ...
      intADCHist->Fill(intADC*0.0000042665);
    }
    fclose(fp);
  }
