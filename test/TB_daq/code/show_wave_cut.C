...
int show_wave_cut(const int runnum, const int Mid, const int channel, const int min, const int max,const int cut)
{
  //int channel;
  int ch_to_plot;
  FILE *fp;
...
//  fp = fopen("cal_sipm_pmt_10k.dat", "rb");
  ...
  for (evt = 0; evt < nevt; evt++) {
    ...
    minadc=4096;
    for (i = 0; i < 1023; i++) {
      if ((i!=0)&&(minadc>adc[i*32+ch_to_plot])){
        minadc=adc[i*32+ch_to_plot];
      }
      plot->Fill(i, adc[i * 32 + ch_to_plot]);
    }
    if (minadc>cut) continue;
