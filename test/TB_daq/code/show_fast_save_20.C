...
int show_fast_save_20(const int runnum, const int mid, const int ch)
{
	int channel;
	int ch_to_plot;
	FILE *fp;
	...
	if (ch < 1)
		...
		ch_to_plot = ch - 1;
	sprintf(outname,"fast_%d_571_%d_%d.root",runnum,mid,ch);
	...
	for (run_max=runnum;run_max<572;run_max++){
