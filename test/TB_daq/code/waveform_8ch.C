...
int waveform_8ch(const int runnum, const int Mid, const int min, const int max, const int maxevt)
{
  int ch_to_plot;
  int channel;
  FILE *fp;
  ...
  TCanvas *c1 = new TCanvas("c1", "CAL DAQ", 800, 400);
...
//
    ...
    c1->SaveAs((TString)("Run_" + std::to_string(runnum) + "_waveform_" + std::to_string(evt) + ".png"));
    //printf("Continue? ");
    //scanf("%d", &cont);
