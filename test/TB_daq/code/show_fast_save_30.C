...
int show_fast_save_30(const int runnum, const int runend, const int mid, const int ch)
{
	int channel;
	int ch_to_plot;
	FILE *fp;
	...
	if (ch < 1)
		...
		ch_to_plot = ch - 1;
	sprintf(outname,"fast_%d_%d_%d_%d.root",runnum,runend,mid,ch);
	TFile *fp_root = new TFile(outname,"recreate");  
	TCanvas *c1 = new TCanvas("c1", "CAL DAQ", 800, 800);
	c1->Divide(1, 2);
		TH1F *plot_e = new TH1F("plot_e", "Energy", 1000, 0, 1000000); 
		TH1F *plot_t = new TH1F("plot_t", "Timing", 1000, 0, 16000); 
		plot_e->Reset();
	plot_t->Reset();

	// get # of events in file
	for (run_max=runnum;run_max<runend;run_max++){
		for (filenum=0;filenum < 200;filenum++){
			...
			sprintf(filename,"/Volumes/HDD_16TB_1/HDD_Run_%d/Run_%d_Fast/Run_%d_Fast_MID_%d/Run_%d_Fast_MID_%d_FILE_%d.dat",run_max,run_max,run_max,mid,run_max,mid,filenum);
