...
int show_wave_png(const int runnum, const int Mid, const int channel, const int min, const int max, const int save_evt)
{
  //int channel;
  int ch_to_plot;
  FILE *fp;
...
//  fp = fopen("cal_sipm_pmt_10k.dat", "rb");
  ...
  for (evt = 0; evt < save_evt; evt++) {
    ...
    c1->Modified();
    c1->Update();
    c1->SaveAs((TString)("Waveform_" + std::to_string(runnum) + "_" + std::to_string(evt) + ".png"));
    // printf("Continue?");
    // scanf("%d", &cont);
