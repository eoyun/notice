...
int waveform_16ch_avg_2(const int runnum, const int Mid)
{
  int ch_to_plot;
  int channel;
  FILE *fp;
  ...
  for (evt = 0; evt < nevt; evt++){
    ...
    for(i=0; i<16; i++){
      ...
      //plot.at(i)->GetYaxis()->SetRangeUser(3700., 3800.);

      //plot.at(i)->GetYaxis()->SetRangeUser(plot.at(i)->GetBinContent(100) - 60, plot.at(i)->GetBinContent(100) + 40);
      plot.at(i)->GetYaxis()->SetRangeUser(3200,4000);
      c1->cd(i+1);
      plot.at(i)->Draw("HIST");
    }

   //c1->SaveAs("./plots/Avg_Run_" + (TString)(std::to_string(runnum)) + "_16ch.png");
  // afile->Close();
