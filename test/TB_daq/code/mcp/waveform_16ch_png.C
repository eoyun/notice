...
int waveform_16ch_png(const int runnum, const int Mid, const int min, const int max, const int maxevt)
{
  int ch_to_plot;
  int channel;
  FILE *fp;
  ...
  TCanvas *c1 = new TCanvas("c1", "CAL DAQ", 700, 700);
...
//    plot.at(i)->write();
    ...
    c1->Modified();
    c1->Update();
    c1->SaveAs((TString)("Run_" + (TString)std::to_string(runnum) + "_waveform_" + (TString)std::to_string(evt) + ".png"));
    // printf("Continue? ");
    // scanf("%d", &cont);
