...
int show_fast_cut(const int runnum, const int mid, const int ch)
{
  int channel;
  int ch_to_plot;
  FILE *fp;
 ...
 // printf("Channel to plot(1~32) : ");
  ...
  if (ch < 1)
    ...
    ch_to_plot = ch - 1;
  sprintf(outname,"cutfast_%d_%d_%d.root",runnum,mid,ch);
  TFile *fp_root = new TFile(outname,"recreate");  
  TCanvas *c1 = new TCanvas("c1", "CAL DAQ", 800, 800);
  c1->Divide(1, 2);
  TH1F *plot_e = new TH1F("plot_e", "Energy", 1010000,-10000, 1000000); 
  ...
  for (filenum=0;filenum < 200;filenum++){
    ...
    for (evt = 0; evt < nevt; evt++) {
      ...
      if(timing>12800&&timing<15000)plot_e->Fill(energy);
      plot_t->Fill(timing);
