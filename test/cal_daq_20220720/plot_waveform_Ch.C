...
int plot_waveform_Ch(const TString filename, const int fevt, const int fch,const int min, const int max)
{
  int channel;
  int ch_to_plot;
  FILE *fp;
...
//    ch_to_plot = channel - 1;
  ...
  TH1F *plot = new TH1F( "plot", Form("Waveform ch%d (%d)", fch, fevt), 1023,0,1023);
  plot->SetStats(0);  
