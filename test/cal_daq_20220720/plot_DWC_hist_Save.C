...
int plot_DWC_hist_Save(const TString filename, const int dwcNum)
{
  int channel[4];
  int ch_to_plot;
  FILE *fp;
...
//  }
  ...
  TFile *tfile = new TFile(filename+"_DWCHist.root","RECREATE");
  TCanvas *c1[nevt];

  for(i = 0 ; i < nevt ; i ++)
  {
    c1[i] = new TCanvas(Form("c%d",i), Form("DWCwaveform_%devt",i) , 1800, 1000);
    c1[i]->Divide(2,2,0.001,0.001);
  }
...
//		if(j==21 || j == 22 || j ==23) cout << j << " th channel : " << adc[i * 32 + j]<< endl;
          if( peak[j] > adc[i * 32 + channel[j]]) peak[j]=adc[i * 32 + channel[j]];
