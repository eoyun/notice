...
int main(int argc, char *argv[])
{
  ...
  // open daq
  for (daq=0;daq<num_of_daq-1;daq++){
    CALDAQopen(mid[daq]);
    printf("daq num is %d\n",mid[daq]);
  }
...
//    CALTCBsend_TRIG(sid);  // optional software trigger
	...
	//printf("%ld\n",data_size);
        //printf("%f %ld\n",(double)(clock()-start)/CLOCKS_PER_SEC, data_size);
      }


      //if (data_size >262100) is_first =1;      
	...
	for (daq=0;daq<num_of_daq-1;daq++){
       ...
       run = CALDAQread_RUN(mid[0]);//check run
      }
    }
