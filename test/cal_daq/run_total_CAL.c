...
int main(int argc, char *argv[])
{
  ...
  //sprintf(filename,"/media/yu/Expansion/DAQ_data/220602/muon_06_02_%d.dat",run_num);
  //sprintf(filename,"/media/yu/Expansion/DAQ_data/220707/data/%s.dat",arg_filename);
  sprintf(filename,"cal_%s.dat",arg_filename);
  fp = fopen(filename, "wb");
...
//    CALTCBsend_TRIG(sid);  // optional software trigger
    data_size = CALDAQread_DATASIZE(mid[0]);
    if (data_size){
      num_of_data_in_buf = data_size/64;
      evt = evt + (data_size/64);
    }
    for (idata=0;idata<num_of_data_in_buf;idata++){
      for (daq=0;daq<num_of_daq;daq++){
        //if (daq == 0) data_size = CALDAQread_DATASIZE(mid[daq]);
        if (data_size > BUF_SIZE)
          data_size = BUF_SIZE;
          //printf("buffer is full\n");
        if (data_size) {
          printf("data size = %ld\n", data_size);
          CALDAQread_DATA(mid[daq], 64, data);
