...
void integral_base(std::string fileNum, std::string pedNum, enum runMode mode) {
    ...
    for ( int i = 0; i < chInput.size(); i++ ) {
        // TH1F* plot = new TH1F((TString)("ch"+std::to_string(chInput.at(i))), (TString)(plotName.at(i)+";#font[42]{Integral(ADC)};#font[42]{evts}"), 200, -102400., 3000000.);
        TH1F* plot = new TH1F((TString)("ch"+std::to_string(chInput.at(i))), (TString)(plotName.at(i)+";#font[42]{Integral(ADC)};#font[42]{evts}"), 100, 1200000., 2500000.);
    ...
    for (int evt = 0; evt < nevt; evt++) {
        ...
        if (mode == runMode::kPrompt || mode == runMode::kFull) {
            ...
            for (int chInt = 0; chInt < chInput.size(); chInt++ ) {
                plotCol.at(chInt)->Fill((std::accumulate(waveformCol.at(chInt).begin(), waveformCol.at(chInt).end(), 0.f) ));
                // if ( (std::accumulate(waveformCol.at(chInt).begin(), waveformCol.at(chInt).end(), 0.f) / 100.) > 12000. ) std::cout << evt << std::endl;
            }     
        }
    ...
    c->cd();
    TPad* pad_LB = new TPad("pad_LB","pad_LB", 0.01, 0.01, 0.5, 0.99 );
    pad_set(pad_LB);

    c->cd();
    TPad* pad_RB = new TPad("pad_RB","pad_RB", 0.5, 0.01, 0.99, 0.99 );
    pad_set(pad_RB);
    ...
    // plotCol.at(1)->SetTitle((TString)std::to_string(nevt));

    c->cd(); pad_LB->cd(); plotCol.at(1)->Draw("Hist");
    c->cd(); pad_RB->cd(); plotCol.at(2)->Draw("Hist");
