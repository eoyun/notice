...
void Run(TString fileNum, TString pedNum, enum discriminator opt, enum runMode mode) {
	...
	if( opt == discriminator::kIntegral &&  mode == runMode::kNoPed ) gROOT->ProcessLine(".x integral_base.cc(\""+fileNum+"\", \""+pedNum+"\", kNoPed)");
	...
	if( opt == discriminator::kWaveform &&  mode == runMode::kFull ) gROOT->ProcessLine(".x plot_waveform_4ch.cc(\""+fileNum+"\", kFull)");
	if( opt == discriminator::kWaveform &&  mode == runMode::kPrompt ) gROOT->ProcessLine(".x plot_waveform_4ch.cc(\""+fileNum+"\", kPrompt)");

	if( opt == discriminator::kTriggerTime &&  mode == runMode::kPrompt ) gROOT->ProcessLine(".x trigger_time.cc(\""+fileNum+"\", \""+pedNum+"\", kPrompt)");
