...
int plot_waveform_root_sw(void)
{
  int channel;
  int ch_to_plot;
  FILE *fp;
  ...
  //TFile *waveform = new TFile("waveform_1_12.root","recreate");
  TH1F *peak_histo = new TH1F("peak_histo","peak;ADC;evt",256,0,4095); 
  plot->SetStats(0);

  // get # of events in file
  fp = fopen("cal_1800_25.dat", "rb");
  ...
  for (evt = 0; evt < nevt; evt++) {
    ...
    integral=0;
    for (j=1;j<151;j++){
      pedestal_tmp += adc[j*32+ch_to_plot];
    }
    //for (k=1;k<1001;k++){
    //  peak_histo->Fill(-(adc[i * 32 + ch_to_plot]-pedestal_tmp/150));
    //}
    for (i = 0; i < 1023; i++) {
      plot->Fill(i,-(adc[i * 32 + ch_to_plot]-pedestal_tmp/150));
//if (adc[i * 32 + ch_to_plot] < 3000) printf("abnormal value = %d @ %d\n", adc[i * 32 + ch_to_plot], evt);
    }
    //waveform->WriteTObject(plot);
    //plot->GetXaxis()->SetRangeUser(0,1000);
    printf("%f",plot->GetMaximum());
    peak_histo->Fill(plot->GetMaximum());
    // comment out below line to see the ADC peak plot only
    plot->Draw("hist");
    c1->Modified();
    c1->Update();
  ...
  peak_histo->Draw("hist");
  //c1->SaveAs("peak_histo.png");/
  //waveform->Close();
  //peak_histo->Draw("hist");
  fclose(fp);
